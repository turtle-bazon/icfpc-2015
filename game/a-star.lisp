...
(defmethod transition-better-p (end-pos)
  (let ((ht (make-hash-table :test 'equal)))
    (flet ((distinct-count (script)
             (clrhash ht)
             (iter (for subscript in script)
                   (setf (gethash subscript ht) t))
             (hash-table-count ht)))
      (lambda (trans-a trans-b)
        (let ((uniq-count-a (distinct-count (a*-st-script trans-a)))
              (uniq-count-b (distinct-count (a*-st-script trans-b))))
